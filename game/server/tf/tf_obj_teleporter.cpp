...
bool CObjectTeleporter::IsReady( void )
{
	...
	return GetState() != TELEPORTER_STATE_BUILDING && !IsDisabled();
}
...
void CObjectTeleporter::RecieveTeleportingPlayer( CTFPlayer* pTeleportingPlayer )
{
	...
	if ( pTeleportingPlayer )
	{
		...
		if ( bClear )
		{
			...
			{
				hPlayersToKill[player]->TakeDamage( CTakeDamageInfo( pTeleportingPlayer, pTeleportingPlayer, 1000, DMG_CRUSH ) );
			}
	...
	case TELEPORTER_STATE_SENDING:
		{
			...
			m_flCurrentRechargeDuration = TELEPORTER_RECHARGE_DURATION;
...
void CObjectTeleporter::Explode( void )
{
	...
	if ( pMatch )
	{
		int iHealth = pMatch->GetBaseHealth();
