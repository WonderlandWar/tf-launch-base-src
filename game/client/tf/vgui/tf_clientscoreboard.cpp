...
void CTFClientScoreBoardDialog::UpdatePlayerDetails()
{
	...
	//TF2007: I'm not sure if this is a good place to change the class image, but it works
	int nClass = pSelectedPlayer->GetPlayerClass()->GetClassIndex();
	m_pClassImage->SetImage( pSelectedPlayer->GetTeamNumber() == TF_TEAM_RED ? g_szRedClassImages[ nClass ] : g_szBlueClassImages[ nClass ]);
