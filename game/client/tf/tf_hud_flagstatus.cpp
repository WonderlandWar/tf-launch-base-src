...
void CTFHudFlagObjectives::UpdateStatus( C_BasePlayer *pNewOwner /*= NULL*/, C_BaseEntity *pFlagEntity /*= NULL*/ )
{
	...
	else
	{
		...
		// TF2007:
		// HACK: This fixes a crash caused by using a 2008 version of HudObjectTimePanel.res (not HudObjectFlagPanel.res!)
		// I have no idea what causes the crash, but constantly dynamically casting can't be good for perf
		m_pCapturePoint = dynamic_cast<CTFArrowPanel *>( FindChildByName( "CaptureFlag" ) );
