...
void CTFWinPanel::FireGameEvent( IGameEvent * event )
{
	...
	else if ( Q_strcmp( "teamplay_win_panel", pEventName ) == 0 )
	{
		...
		ImagePanel *pBlueBGPanel = m_pTeamScorePanel->FindControl< ImagePanel >( "BlueScoreBG", true );
		Assert( pBlueBGPanel );
		ImagePanel *pRedBGPanel = m_pTeamScorePanel->FindControl< ImagePanel >( "RedScoreBG", true );
		Assert( pRedBGPanel );
		if( !pBlueBGPanel || !pRedBGPanel )
			return;

		//pBlueBGPanel->SetBorder( pScheme->GetBorder( "TFFatLineBorderBlueBG" ) );
		//pRedBGPanel->SetBorder( pScheme->GetBorder( "TFFatLineBorderRedBG" ) ) ;
		...
		{
		case TF_TEAM_BLUE:
			pBGPanel->SetImage("../hud/winpanel_blue_bg_main.vmt");
