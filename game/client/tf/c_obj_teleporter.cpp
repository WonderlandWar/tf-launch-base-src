...
void C_ObjectTeleporter::StartActiveEffects()
{
	...
	m_pSpinSound = controller.SoundCreate( filter, entindex(), "Building_Teleporter.Spin" );
...
void C_ObjectTeleporter::GetTargetIDDataString( OUT_Z_BYTECAP(iMaxLenInBytes) wchar_t *sDataString, int iMaxLenInBytes )
{
	...
	{
		const char *pszString = GetObjectMode() == MODE_TELEPORTER_ENTRANCE ? "#TF_playerid_teleporter_exit_noentrance" : "#TF_playerid_teleporter_entrance_noexit";
		g_pVGuiLocalize->ConstructString( sDataString, iMaxLenInBytes, g_pVGuiLocalize->Find( pszString ), 0);
	}
}
