...
{
	const char *pszCustomDir = "resource/UI/destroy_menu";
	...
	for ( int i=0; i<NUM_ENGY_BUILDINGS; ++i )
	{
		...
		pNumberLabel = dynamic_cast<CTFLabel *>( m_pActiveItems[i]->FindChildByName( "NumberLabel" ) );
		if ( pNumberLabel )
		{
			pNumberLabel->SetText( VarArgs( "%d", i + 1 ) );
		}
		pNumberLabel = dynamic_cast<CTFLabel *>( m_pInactiveItems[i]->FindChildByName( "NumberLabel" ) );
		if ( pNumberLabel )
		{
			pNumberLabel->SetText( VarArgs( "%d", i + 1 ) );
		}
		pNumberLabel = dynamic_cast<CTFLabel *>( m_pUnavailableItems[i]->FindChildByName( "NumberLabel" ) );
		if ( pNumberLabel )
		{
			pNumberLabel->SetText( VarArgs( "%d", i + 1 ) );
		}
