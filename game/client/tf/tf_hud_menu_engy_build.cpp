...
{
	// TF2007: TODO: Support the steam controller?
	...
	{
		const char* pszCustomDir = "resource/UI/build_menu";
		...
		for ( int i=0; i<NUM_ENGY_BUILDINGS; ++i )
		{
			...
			pNumberLabel = dynamic_cast< CTFLabel * >( m_pAvailableObjects[i]->FindChildByName( "NumberLabel" ) );
			if ( pNumberLabel )
			{
				pNumberLabel->SetText( VarArgs( "%d", i+1 ) );
			}
			// Set the Numerical Number
			pNumberLabel = dynamic_cast<CTFLabel *>( m_pAlreadyBuiltObjects[i]->FindChildByName( "NumberLabel" ) );
			if ( pNumberLabel )
			{
				pNumberLabel->SetText( VarArgs( "%d", i+1 ) );
			}
			// Set the Numerical Number
			pNumberLabel = dynamic_cast<CTFLabel *>( m_pCantAffordObjects[i]->FindChildByName( "NumberLabel" ) );
			if ( pNumberLabel )
			{
				pNumberLabel->SetText( VarArgs( "%d", i+1 ) );
			}
			// Set the Numerical Number
			pNumberLabel = dynamic_cast<CTFLabel *>( m_pUnavailableObjects[i]->FindChildByName( "NumberLabel" ) );
			if ( pNumberLabel )
			{
				pNumberLabel->SetText( VarArgs( "%d", i+1 ) );
			}
...
void CHudMenuEngyBuild::SetVisible( bool state )
{
	if ( state == true )
	{
		...
		{
			// TF2007: Not sure what to do here, maybe check if it hasn't been built already?
			//InvalidateLayout( true, true );
		}
