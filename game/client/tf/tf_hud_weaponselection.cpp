	...
	for ( int i = 0; i < m_iMaxSlots; i++ )
	{
		...
		if ( i == iActiveSlot )
		{
			for ( int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS; slotpos++ )
			{
				...
				{
					m_pPanels[i]->SetBorder( pScheme->GetBorder("TFFatLineBorderBlueBG") );
				}
				else
				{
...
void CHudWeaponSelection::DrawSelection( C_BaseCombatWeapon *pSelectedWeapon )
{
	...
	{
		int xpos, ypos;
		m_pPanels[i]->GetPos( xpos, ypos );
		
		int wide, tall;
		m_pPanels[i]->GetSize( wide, tall );
		...
		if ( i == iActiveSlot )
		{
			...
			for ( int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS; slotpos++ )
			{
				...
				// DrawWeaponTexture must be placed before the DrawString call, otherwise the weapon will be on top of the text
				DrawWeaponTexture( pPlayer, pSelectedWeapon, xpos, ypos, m_flLargeBoxWide, m_flLargeBoxTall );
		...
		else
		{
			...
			if ( GetFirstPos( i ) )
			{
				...
				DrawWeaponTexture( pPlayer, pWeapon, xpos, ypos, m_flSmallBoxWide, m_flSmallBoxTall );
