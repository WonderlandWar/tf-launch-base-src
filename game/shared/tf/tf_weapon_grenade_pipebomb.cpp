																	...
																	int iPipeBombType )
{
	...
	if ( pGrenade )
	{
		...
		pGrenade->SetDamage( pGrenade->GetDamage() );
...
void CTFGrenadePipebombProjectile::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	bool bIsDynamicProp = ( NULL != dynamic_cast<CDynamicProp *>( pHitEntity ) ); // TF2007: not sure what to do with this
...
{
	SetContextThink( &CTFGrenadePipebombProjectile::ArmThink, gpGlobals->curtime + tf_grenadelauncher_livetime.GetFloat(), "ARM_THINK" );
}
